...
`capped` is the final capped score and the final score before display formatting. It retains full internal numeric precision. The displayed final score is `format(capped)`, normally rendered with two decimals. `capped` is the minimum across the evidence-adjusted score and the three individual caps; it must not be confused with `governanceCap`, `nhiCap`, or `traceCap`.
...
- Scoring values are JavaScript `Number` values and therefore follow IEEE-754 double-precision floating-point semantics.
- Scoring uses the implemented JavaScript arithmetic and `Math` operations, including `Math.min`, `Math.max`, and subtraction-based sort comparators.
- Intermediate values are not rounded before later scoring calculations.
- UI and diagnostic display formatting use JavaScript `Number(value).toFixed(2)`.
- A cross-language implementation must reproduce these numeric and formatting semantics to guarantee identical edge-case results.
...
The blocker candidates use adjusted domain values. They are constructed in this exact order:
...
They are then sorted in ascending raw-value order:

```text
blockers.sort((a, b) => a.value - b.value)
```

When two blockers have identical raw values, the comparator returns zero. Equal-valued blockers retain their original candidate order because JavaScript `Array.prototype.sort` is stable. This deterministic equal-value behavior therefore relies on JavaScript stable-sort behavior. The first blocker in the resulting order becomes `primaryBlocker`; when multiple blockers have identical lowest raw values, the earliest one in the candidate order above becomes primary.
...
Possible-move candidates begin as the currently disabled levers produced by filtering the `improvements` array. Filtering preserves the exact `improvements` array order:
...
Selected levers are omitted, and the remaining candidates retain their relative order. For each disabled improvement lever, the simulator temporarily enables that lever and recalculates adjusted domains, raw capability, evidence penalty, traceability penalty, evidence-adjusted score, and final capped score using the same formulas above.
...
When two possible moves have identical gains, the comparator returns zero. Equal-gain moves retain their filtered `improvements` array order because JavaScript `Array.prototype.sort` is stable. The top-three slice is applied after this stable descending-gain sort, so original improvements-array order determines which equal-gain moves appear first and can determine which tied move remains within the first three.
...
Graph-domain rows use this exact `domains` array iteration order:
...
The weakest graph domain is the first domain retained while reducing adjusted values with a strict lower-than comparison. When values tie, the strict comparison is false, so the first encountered tied domain remains weakest. The strongest graph domain follows the same rule with a strict greater-than comparison, retaining the first encountered tied domain as strongest. Graph tie ordering therefore follows the domain order above.
